...
| Deploy to prod | **Render (primary):** push to the connected branch, then trigger a deploy in the Render dashboard (or `render deploys create`) — `autoDeploy` is off; see RENDER_MIGRATION.md for the full runbook. **(rollback until cutover) DigitalOcean:** SSH to 161.35.13.81 as root. `cd /opt/pear-autoreplies/app && git pull && docker compose up -d`. To rebuild a single service (e.g. after app code changes): `docker compose build web && docker compose up -d web`. After Caddy config changes: `docker compose build caddy && docker compose up -d caddy`. |
| Force harness mailbox refresh | Harness caches the monitored-mailbox list for 6 hours. After enabling a new inbox in Airtable: **DO:** `docker compose kill -s HUP harness-poller` on the droplet. **Render:** SIGHUP is not exposed — Restart the `pear-autoreplies-harness-poller` service from the dashboard (startup re-reads the mailbox list) or wait out the 6h cache. |
...
- **Caddy runs from a custom image** (`Dockerfile.caddy`), not stock `caddy:2-alpine`. The image bundles the `caddy-ratelimit` plugin via `xcaddy`. The `rate_limit` directive in the Caddyfile requires this — it is NOT a built-in Caddy directive. **(DO only)**
- **Caddy / `Dockerfile.caddy` / `Caddyfile` are retained for DO rollback ONLY and are unused on Render.** Render terminates TLS and handles routing natively; the custom Caddy image cannot run on Render without its `xcaddy`-built binary. The Caddy `/admin/*` rate limiter is replaced by `src/autoreplies/ratelimit.py` app-level middleware (`RateLimitMiddleware`), which covers `/admin/*`.
