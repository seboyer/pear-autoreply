"""SQLite-backed dedup and mailbox-cursor state for the production poller.

Same logic as harness/state.py, but in a separate file so the two state stores
never interact and the distinctness invariant stays clean. The production poller
targets primary mailboxes; the harness targets autoreply mailboxes.
"""
...
class PollerState:
    """Lightweight SQLite wrapper for production poller state.
    ...
    def mark_processed(
        ...
        self._conn.execute(
            ...
            INSERT OR REPLACE INTO processed_messages
                (gmail_message_id, mailbox_email, airtable_inquiry_id, processed_at, error)
            VALUES (?, ?, ?, ?, ?)
            """,
            (message_id, mailbox, inquiry_id, now, error),
        )
        self._conn.commit()
