...
| Item | Resolution |
|---|---|
| Repeated / duplicate inquiries (logged 2026-06-11) | **✅ RESOLVED (2026-08-02).** StreetEasy duplicate-sends the same inquiry (observed 2× per prospect, ~minutes apart) and a prospect's conversation follow-ups also arrive as `New Message From <name>`. Original dedup was per Gmail message-id, so each triggered its own auto-reply. Fixed in two phases at the dispatch layer (not the parser), as Sam specified: **Phase 1** (PR #27) suppresses true re-sends by content fingerprint within `DEDUP_WINDOW_SECONDS` (1h); **Phase 2** (PR #28) detects a repeat inquiry from the same person + same agent mailbox within `REPEAT_INQUIRY_WINDOW_SECONDS` (14d) via the shared identity RPC and **swaps in a shorter repeat template** rather than re-firing first-touch. Gated by `REPEAT_INQUIRY_MODE` (`off`/`observe`/`enforce`); observe-validated over 47 days (20.1% of leads flagged). See [DEDUPE_PHASE2_HANDOFF.md](./DEDUPE_PHASE2_HANDOFF.md). |
